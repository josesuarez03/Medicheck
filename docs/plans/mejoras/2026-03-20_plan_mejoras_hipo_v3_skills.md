...
| 2 | Tabla AuditLog con firma criptográfica | Django | ✅ RESUELTO · Fase 2 |
| 3 | Cifrado de campos clínicos en PostgreSQL | Django | ✅ RESUELTO · Fase 2 |
| 4 | Cifrado de mensajes clínicos en MongoDB | Flask | ✅ RESUELTO · Fase 2 |
...
| 7 | Corrección de la clase Encryption de Flask | Flask | ✅ RESUELTO · Fase 2 |
...
- **Resultado cerrado:** se añadió trazabilidad auditable para cambios clínicos, se cifraron datos sanitarios en PostgreSQL y MongoDB y se corrigió la capa de cifrado usada por Flask para evitar inconsistencias operativas.
...
- `#44` (estabilización flujo paciente) → después de `#43`, porque reutiliza el flujo ETL ya corregido y añade guardas para no reejecutarlo en el mismo episodio
- `#45` (preservación de contexto clínico global) → después de `#43` y coordinada con `#44`, porque necesita una ETL estable y reglas claras para decidir cuándo fusionar, conservar o ampliar contexto clínico persistente
...
| `frontend/src/...` + vistas de dashboard/historial/configuración | **#44** |
| `flask-services/src/services/chatbot/...` + detección de finalización/ETL | **#44** |
| `django_services/users/...` + estado de perfil/completitud | **#44** |
| `flask-services/src/services/...` + RAG/context manager/extracción clínica | **#45** |
| `django_services/users/models.py` + merge de ficha clínica persistente | **#45** |

---

## Bloque 12 — Estabilización del flujo paciente (NUEVO)

### #44 — Estabilización del flujo paciente: perfil, historial y guard ETL

**Servicio:** Django + Flask + Frontend | **Prioridad:** 🟡 IMPORTANTE

**Problema:** Se han detectado tres fallos relacionados dentro del flujo paciente actual:

1. Tras cambiar la contraseña, el sistema vuelve a pedir completar el perfil aunque el usuario ya lo tenía completo.
2. El dashboard/historial se rompe visualmente cuando llega contenido clínico largo o con markdown crudo.
3. Si el usuario manda un mensaje adicional como `ok`, la ETL puede volver a dispararse aunque el contexto clínico siga siendo el mismo.

#### Bloque de objetivos

- Mantener intacto el estado de perfil completado tras un cambio de contraseña o refresco de sesión.
- Hacer robusto el renderizado del historial y dashboard ante resúmenes clínicos largos, con formato irregular o markdown.
- Introducir un guard funcional para que la ETL solo se relance si hay nueva información clínica relevante o un nuevo episodio.
- Mejorar la trazabilidad para saber por qué se redirige a completar perfil y por qué una ETL se ejecuta o se omite.

#### Impacto funcional

- Reduce fricción en configuración/autenticación.
- Evita degradación visual del dashboard de paciente.
- Elimina ejecuciones ETL duplicadas y artefactos clínicos redundantes.

#### Áreas a revisar

- **Django:** cambio de contraseña, serialización del usuario autenticado, flags de perfil completo y endpoints de sesión/perfil.
- **Flask:** control de finalización, disparo de ETL, gestión de contexto conversacional y deduplicación por episodio.
- **Frontend:** lógica de redirección a completar perfil, renderizado del historial de triajes, tarjetas/resúmenes clínicos y manejo de overflow.

#### Archivos a revisar

- `django_services/users/views.py`
- `django_services/users/serializers.py`
- `django_services/users/models.py`
- `flask-services/src/services/chatbot/application/finalization_service.py`
- `flask-services/src/services/chatbot/application/chat_turn_service.py`
- `flask-services/src/routes/sockets_events.py`
...
**A. Cambio de contraseña no debe reabrir el onboarding**

1. Revisar el flujo de cambio de contraseña y el endpoint de refresco de usuario autenticado.
2. Confirmar qué campo gobierna la redirección a completar perfil (`profile_completed`, campos obligatorios o equivalente).
3. Evitar recomputar ese flag a `false` por pérdida de datos parciales, respuesta incompleta del backend o limpieza incorrecta del estado cliente.
4. Ajustar frontend para redirigir solo cuando falten datos clínicos/identitarios realmente obligatorios.
5. Añadir logging de decisión cuando se fuerce el flujo de completar perfil.

**B. Corregir la rotura visual del historial/dashboard**

1. Normalizar el resumen clínico destinado a vistas compactas para no renderizar markdown crudo (`#`, `**`, etc.).
2. Introducir una variante resumida en texto plano para cards y listados.
3. Aplicar control de `overflow`, salto de línea y truncado multinea en componentes de historial.
4. Verificar que un resumen largo no provoque scroll horizontal en desktop ni rompa el layout.
5. Mantener la versión completa del resumen solo en el detalle, modal o vista extendida si existe.

**C. Guard para impedir relanzamientos ETL espurios**

1. Registrar la última ETL ejecutada por `conversation_id` o `episode_id`.
2. Calcular una firma o hash del contenido clínicamente relevante ya procesado.
3. No relanzar ETL cuando el nuevo mensaje sea de baja señal (`ok`, `vale`, confirmaciones breves) y no cambie los hechos clínicos.
4. Sí relanzar ETL cuando haya síntomas nuevos, cambio de intensidad, duración, medicación, antecedentes o apertura explícita de un nuevo caso.
5. Dejar trazabilidad en logs de la decisión: `etl_triggered` frente a `etl_skipped_same_context`.

#### Criterio de aceptación

- Un usuario con perfil completo cambia la contraseña y no vuelve al flujo de completar perfil.
- El historial de triajes no muestra markdown crudo ni provoca desbordamiento horizontal con resúmenes largos.
- Tras una ETL ya ejecutada, un mensaje como `ok` no dispara una nueva ETL.
- Si el usuario añade información clínica nueva en el mismo chat, la ETL sí puede reejecutarse.
- Los logs permiten distinguir redirecciones legítimas de perfil y ETLs omitidas por contexto repetido.

#### Dependencias

- Depende de `#43` para partir de una ETL ya estable hacia Django.
- Se coordina con `#14` y `#31` porque comparten lógica de finalización y cierre conversacional.
...
Además, el sistema no está componiendo correctamente el contexto que necesita el chatbot para razonar y extraer datos: debe combinar contexto global del usuario, contexto de la conversación actual y contexto de sesiones anteriores.

#### Bloque de objetivos

- Preservar el contexto clínico persistente del paciente cuando una nueva ETL solo aporta una actualización parcial.
- Evitar que campos estables como alergias, medicación crónica, antecedentes u ocupación se borren por omisión o por una extracción incompleta.
- Separar claramente contexto global del paciente, contexto de episodio actual y contexto histórico reutilizable.
- Alimentar el RAG con una composición explícita de:
  - contexto global del usuario
  - contexto de la conversación actual
  - contexto relevante de sesiones anteriores

#### Problema funcional detallado

- La ETL/chatbot está escribiendo snapshots parciales que pueden sobrescribir información válida previa con vacío, `null` o ausencia de dato.
- El merge de ficha clínica no distingue entre:
  - dato nuevo explícito
  - dato no mencionado
  - dato a eliminar deliberadamente
- El RAG y/o el gestor de contexto no mantienen una memoria clínica longitudinal suficientemente estable.

#### Ejemplos observados

- Una entrada del chatbot guarda `ocupacion=Programador` y `allergies` con valor válido, pero una actualización posterior deja `allergies` vacío aunque el paciente sigue teniendo esa alergia.
- La hoja clínica del paciente termina con parte del contexto actualizado, pero pierde campos persistentes que deberían arrastrarse mientras no exista contradicción o eliminación explícita.

#### Áreas a revisar

- **Django:** lógica de `update_from_chatbot_analysis()`, merge de `Patient` con `PatientHistoryEntry`, reglas de sobrescritura y conservación de campos.
- **Flask:** extracción de datos estructurados, construcción de payload ETL, consolidación de contexto y recuperación de memoria previa.
- **RAG/contexto:** composición del prompt y recuperación de memoria longitudinal del usuario más contexto actual de la conversación.
...
- `django_services/users/views.py`
- `flask-services/src/services/process_data/medical_data.py`
- `flask-services/src/services/chatbot/context_manager.py`
- `flask-services/src/services/chatbot/chatbot.py`
- `flask-services/src/services/chatbot/pain_utils.py`
- `flask-services/src/services/chatbot/...` relacionados con memoria, RAG o recuperación de contexto

#### Líneas de implementación

**A. Merge clínico conservador en persistencia**

1. Revisar la actualización de `Patient` desde análisis del chatbot para que un valor vacío no sobrescriba automáticamente un valor persistente válido.
2. Definir semántica explícita por campo:
   - `no informado en esta interacción` -> conservar valor previo
   - `informado con nuevo valor` -> actualizar
   - `eliminación explícita` -> borrar con trazabilidad
3. Aplicar esta regla al menos a `allergies`, `medications`, `medical_history`, `ocupacion` y otros campos persistentes equivalentes.
4. Registrar en auditoría qué campos se conservaron, cuáles se actualizaron y por qué.

**B. Separación entre contexto global y contexto del episodio**

1. Mantener una representación clara de contexto global del paciente separada del resumen clínico de la conversación activa.
2. No degradar el contexto global con inferencias débiles o parciales sacadas de una sola interacción.
3. Permitir que el episodio actual añada síntomas, motivo de consulta y evolución sin borrar antecedentes persistentes.

**C. RAG con memoria compuesta**

1. Construir el contexto del chatbot con tres capas:
   - perfil clínico global del usuario
   - estado y hechos clínicos de la conversación activa
   - memoria relevante de sesiones previas
2. Priorizar hechos persistentes validados del paciente frente a extracciones nuevas no confirmadas.
3. Definir reglas de resolución de conflictos entre memoria previa y nuevos datos.
4. Evitar que ocupación, alergias o antecedentes se usen como sustituto incorrecto del motivo de consulta actual.

**D. Protección ante ETL parciales**

1. Marcar payloads ETL como parciales cuando no traigan todos los campos clínicos persistentes.
2. Impedir que una ETL parcial vacíe la ficha clínica del paciente.
3. Añadir pruebas con secuencias de varias actualizaciones donde la segunda solo aporte parte de la información.

#### Criterio de aceptación

- Si el paciente tiene `allergies="ácaros"` y la nueva interacción no modifica alergias, la ficha clínica mantiene esa alergia.
- Si una nueva interacción aporta solo síntomas y nivel de triaje, no se pierden ocupación, alergias ni antecedentes persistentes.
- El chatbot/RAG puede usar simultáneamente contexto global del usuario, contexto de la conversación actual y memoria relevante de sesiones anteriores.
- Las entradas históricas siguen reflejando lo ocurrido en cada sesión, pero la ficha clínica consolidada no pierde contexto por sobrescrituras parciales.
- Existe trazabilidad suficiente para explicar por qué un campo se conservó, se actualizó o se eliminó.
