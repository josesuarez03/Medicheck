...
class EncryptedTextField(models.TextField):
    ...
    def get_prep_value(self, value):
        ...
        if isinstance(value, str) and value.startswith(ENCRYPTED_VALUE_PREFIX):
            return value
        encrypted = Encryption(key=_field_encryption_key().encode("utf-8")).encrypt_string(value)
    ...
    def to_python(self, value):
        ...
        return Encryption(key=_field_encryption_key().encode("utf-8")).decrypt_string(encrypted_value)
